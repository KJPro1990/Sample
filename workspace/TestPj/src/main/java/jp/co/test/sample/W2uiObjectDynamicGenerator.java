...
public class W2uiObjectDynamicGenerator {
	...
	record FormInfo(String name, String box, List<FormFields> fields) {
		FormInfo(String name, List<FormFields> fields) {
	...
	public static List<FormInfo> generateFormInfo() {
		return List.of(new FormInfo("form1", generateFormFields()),
				new FormInfo("form2", generateFormFields()),
				new FormInfo("form3", generateFormFields()));
	}

	private static List<FormFields> generateFormFields() {
	...
	record GridInfo(String name, String box, List<GridColumns> columns, GridShowProperty show, List<GridSearchCondition> searchs) {
		GridInfo(String name, List<GridColumns> columns, GridShowProperty show, List<GridSearchCondition> searchs) {
			this(name, "#" + name, columns, show, searchs);
		}
	}

	record GridShowProperty(Boolean toolbar, Boolean footer, Boolean lineNumbers) {
	}

	record GridColumns(String field, String caption, String size, Boolean sortable) {
	}

	public static List<GridInfo> generateGridInfo() {
		return List.of(new GridInfo("grid1", generateGridColumns(), new GridShowProperty(true, true, true), generateGridSearches()),
				new GridInfo("grid2", generateGridColumns(), new GridShowProperty(true, true, true), generateGridSearches()),
				new GridInfo("grid3", generateGridColumns(), new GridShowProperty(true, true, true), generateGridSearches()));
	}

	private static List<GridColumns> generateGridColumns() {
	...
	private static List<GridSearchCondition> generateGridSearches() {
